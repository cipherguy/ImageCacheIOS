...
-(void)setCostLimit:(int)size
{
    [self.cache setTotalCostLimit:size];
}

#pragma mark - NSObject

- (id)init {
    NSLog(@" You must initalize cache using `initWithName:`.");
    return nil;
}
...
+ (CacheContainer *)sharedCache {
    static CacheContainer *sharedCache = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedCache = [[CacheContainer alloc] initWithName:@"com.cipher.container.shared" directory:nil Size:10];
    });
    return sharedCache;
}
...
- (instancetype)initWithName:(NSString *)name Size:(int)size {
    return [self initWithName:name directory:nil Size:size];
}

- (instancetype)initWithName:(NSString *)name directory:(NSString *)directory Size:(int)size {
    NSParameterAssert(name);
    
    if ((self = [super init])) {
        self.name = [name copy];
        self.cache.name = self.name;
        self.cache.totalCostLimit = size;
        ...
        if (!directory) {
            ...
            directory = [cachesDirectory stringByAppendingFormat:@"/com.cipher.container/%@", self.name];
        }
